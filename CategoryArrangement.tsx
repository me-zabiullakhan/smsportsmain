...
const CategoryArrangement: React.FC = () => {
    ...
    const colCount = isAllCategories 
        ? Math.max(6, ...categories.map(cat => {
            ...
            // Calculate total slots based on default 6-col segments or custom config
            const cols = c.cols || 6;
            const rows = c.rows || Math.ceil(tr / cols);
            return rows * cols;
          }))
        : (config.cols || 6);
                                            ...
                                            <tr>
                                                ...
                                                {isAllCategories ? (
                                                    Array.from({ length: colCount }).map((_, cIdx) => (
                                                        <th key={cIdx} className={`p-5 border text-[14px] font-black uppercase tracking-widest shine-effect ${isDark ? 'bg-zinc-900/90 border-accent/20' : 'bg-gray-50 border-blue-100'}`}>
                                                            <span className="golden-text">Slot {cIdx + 1}</span>
                                                        </th>
                                                    ))
                                                ) : (
                                                    Array.from({ length: colCount }).map((_, cIdx) => (
                                        ...
                                        <tbody>
                                            {Array.from({ length: rowCount }).map((_, rIdx) => {
                                                ...
                                                const rowLabel = isAllCategories 
                                                    ? (categories[rIdx]?.name || `CAT_${rowNum}`) 
                                                    : (isAllrounderTable ? (categories[rIdx]?.name || `EXTRA_${rowNum}`) : `${prefix}${rowNum}`);
                                                return (
                                                    <tr key={rowLabel}>
                                                                    ...
                                                                    <div className="flex gap-1">
                                                                        ...
                                                                        {isAllCategories && (
                                                                            <button 
                                                                                ...
                                                                                className={`p-1 rounded transition-all group ${isDark ? 'bg-red-500/10' : 'bg-red-100'}`}
                                                                            >
                                                                                <Trash2 className={`w-2.5 h-2.5 transition-all text-red-500`} />
                                                                            </button>
                                                                        )}
                                                                    </div>
                                                                )}
                            ...
                            <table className="w-full border-collapse relative z-10">
                                <thead>
                                    <tr className="bg-zinc-900/90">
                                        <th className="w-32 p-6 border border-amber-500/20 text-xl font-black text-amber-500 uppercase tracking-widest bg-gradient-to-b from-zinc-800 to-zinc-900">
                                            {isAllCategories ? 'CATEGORY' : (isAllrounderTable ? 'CATEGORY' : '#')}
                                        </th>
                                        {isAllCategories ? (
                                            Array.from({ length: colCount }).map((_, cIdx) => (
                                                <th key={cIdx} className="p-6 border border-amber-500/20 text-xl font-black text-amber-500 uppercase tracking-widest bg-gradient-to-b from-zinc-800 to-zinc-900">
                                                    Slot {cIdx + 1}
                                                </th>
                                            ))
                                        ) : (
                                            Array.from({ length: colCount }).map((_, cIdx) => (
                                                <th key={cIdx} className="p-6 border border-amber-500/20 text-xl font-black text-amber-500 uppercase tracking-widest bg-gradient-to-b from-zinc-800 to-zinc-900">{cIdx + 1}</th>
                                            ))
                                        )}
                                    </tr>
                                </thead>
